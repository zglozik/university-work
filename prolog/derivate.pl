...
fsimplify(F, H) :-
	findall(Y, permute(F,Y), A),
	iterate_simplify_list(A, [B|Bs]),
	complexity(B, BC),
	min_complexity(Bs, B, BC, H).

min_complexity([G2|Gs], _, G1C, H) :-
	complexity(G2, G2C),
	G2C =< G1C,
	!,
	min_complexity(Gs, G2, G2C, H).

min_complexity([_|Gs], G1, G1C, H) :-
	!,
	min_complexity(Gs, G1, G1C, H).

min_complexity([], G, _, G).

/* generates simplified expressions */

iterate_simplify_list([], []).

iterate_simplify_list([A|As], [B|Bs]) :-
	iterate_simplify(A, B),
	iterate_simplify_list(As, Bs).
