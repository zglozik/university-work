...
% Disadvantage: solve_puzzle/2 forces meta-calls, that is, interpreted calls.
...
*/
     dag_path(X,X,[X]).
     dag_path(X,Y,[X|P]) :- edge(X,N), dag_path(N,Y,P).
...
legal_action(to_block(Block1,Y,Block2),State) :- 
   on(Block1,Y,State), clear(Block1,State), block_(Block2), 
